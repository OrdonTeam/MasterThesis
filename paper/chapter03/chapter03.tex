...
\begin{algorithm}[H]
    ...
    $x\gets 2$\;
    \While{$x <= n$}{
        \For{dla każdego $x$ elementowego podzbioru zbioru argumentów}{
            \If{$MR$ nie zwiera danego podzbioru}{
                zastosuj do $F$ dekompozycję zgodnie ze znaleienoym podzbiorem\;
                ponów algorytm dla zdekmponowanej funkcji\;
            }
        }
    }
\end{algorithm}
